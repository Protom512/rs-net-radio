...
use record_lib::domain::types::{OutputPath, ProgramTitle, StreamingUrl};
use std::sync::Arc;
...
impl record_lib::domain::service::RecordService for MockRecordService {
    ...
    ) -> Result<record_lib::domain::metadata::RecordingMetadata, record_lib::utils::RecordError>
    {
        // Simulate some work
...
fn create_programs(count: usize) -> Vec<Program> {
    (0..count)
        .map(|i| Program {
            title: ProgramTitle::from(format!("Program {}", i)),
            url: StreamingUrl::from(format!("http://example.com/{}", i)),
            output_path: OutputPath::from(format!("/tmp/{}.m4a", i).as_str()),
        })
        .collect()
}
